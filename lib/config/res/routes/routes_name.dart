class RoutesName {
  // Define route names as static constants
  static const String splash = '/splash';
  static const String onboard = '/onboard';
  static const String crateAccountScreen = '/create-account-screen';
  static const String resumeScreen = '/resume-screen';
  static const String settingScreen = '/setting-screen';
  static const String createResume = '/info-selection-screen';
  static const String bottomNavBarScreen = '/bottom-nav-bar-screen';
  static const String resumeDetailScreen = '/resumeDetailScreen';
  static const String notFound = '/not-found';
}
